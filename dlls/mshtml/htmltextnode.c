...
static void init_char_data(HTMLDOMNode *node, struct CharacterData *ret)
{
    nsresult nsres = nsIDOMNode_QueryInterface(node->nsnode, &IID_nsIDOMCharacterData, (void**)&ret->nschardata);

    if(nsres != NS_OK)
        ret->nschardata = NULL;
    else {
        assert((nsIDOMNode*)ret->nschardata == node->nsnode);

        /* Share reference with nsnode */
        nsIDOMCharacterData_Release(ret->nschardata);
    }
...
HRESULT HTMLCommentElement_Create(HTMLDocumentNode *doc, nsIDOMNode *nsnode, HTMLElement **elem)
{
    ...
    init_char_data(&ret->element.node, &ret->character_data);
