...
DWORD WINAPI SendARP(IPAddr DestIP, IPAddr SrcIP, PULONG pMacAddr, PULONG PhyAddrLen)
{
	...
	do
	{
		...
		if (i == NO_ERROR )
			...
			while ( pCurrInterface )
			{
				...
				while ( pUnicast )
				{
					if ( DestIP == ((struct sockaddr_in *) pUnicast->Address.lpSockaddr)->sin_addr.s_addr )
					{
	...
	src.sin_addr.s_addr = SrcIP;	// del?
	src.sin_port = 10123;		// del?
	dst.sin_family = AF_INET;
	dst.sin_addr.s_addr = DestIP;
	dst.sin_port = 10123;		// del?

	// send UDP
	if( WSAStartup( MAKEWORD( 2, 2 ), &wd ) != 0 )
		return ERROR_NOT_SUPPORTED;
	s = socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP ); // SOCK_RAW, IPPROTO_ICMP
	bind( s, (SOCKADDR *) &src, sizeof(src) );
	i = sendto( s, (const char *) &time_out, 4, 0, (SOCKADDR *) &dst, sizeof(dst) );
	closesocket(s);
    	WSACleanup();
	if( i == SOCKET_ERROR ) // -1
		return ERROR_GEN_FAILURE; // 31
